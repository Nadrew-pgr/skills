...
For HyperFrames, prefer composition pixels for layout and normalized timeline progress for animation math. Keep random-looking motion seeded and deterministic so preview, inspect, and render match.
...
Use parallax to create spatial progress without moving every element. For video, use fewer depth layers with readable hierarchy; background motion can be subtle, but subject motion must carry the story.
...
For video, "camera" often means a wrapper transform or crop window. Do not use element scale as a substitute for spatial entry unless the shot is intentionally graphic.
